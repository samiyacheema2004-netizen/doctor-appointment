...
const pool = new Pool({
    ...
    ssl: process.env.NODE_ENV === "production" || process.env.DATABASE_URL?.includes("render") || process.env.DATABASE_URL?.includes("vercel")
...
pool.connect()
    ...
    .catch(err => console.error("Database connection error on boot:", err.stack));
...
app.post("/api/appointments", async (req, res) => {
    ...
    try {
        ...
        // Save appointment
        const result = await pool.query(
            `INSERT INTO appointments ("doctorId", "patientName", date, time)
             VALUES ($1, $2, $3, $4)
             RETURNING *`,
...
app.put("/api/appointments/:id", async (req, res) => {
    ...
    try {
        ...
        const doctorId = appointment.rows[0].doctorId;

        // Check new slot duplicate
        const existingAppointment = await pool.query(
            `SELECT * FROM appointments
             WHERE "doctorId" = $1 AND date = $2 AND time = $3 AND id != $4`,
            [doctorId, date, time, appointmentId]
        );

        if (existingAppointment.rows.length > 0) {
            return res.status(409).json({ message: "This new time slot is already booked." });
        }
